...
# landsat
dirLandsat = os.path.join(kPath.dirVeg, "RS", "landsat8-30m")
...
# sentinel
dirSentinel = os.path.join(kPath.dirVeg, "RS", "sentinel1-250m")
varS = ["VV", "VH"]
...
# modis
dirAqua = os.path.join(kPath.dirVeg, "RS", "MODIS_quality", "MOD09GA-500m")
dirTerra = os.path.join(kPath.dirVeg, "RS", "MODIS_quality", "MYD09GA-500m")
...
for k, var in enumerate(varF):
    dfM = pd.read_csv(
        os.path.join(kPath.dirVeg, "forcings", "{}.csv".format(var)), index_col=0
    )
...
fileName = os.path.join(kPath.dirVeg, "const", "nlcd_2016_4km.tif")
...
# combine
varX = varSA + varLA + varMA + varF
matX = np.concatenate([matSA, matLA, matMA, matF], axis=-1)
varY = ["LFMC"]
matY = matNFMD[:, :, None]
